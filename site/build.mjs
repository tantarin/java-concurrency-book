...
const home = `<section class="hero"><div class="eyebrow">Открытый учебник · ${chapterCount} глава</div><h1>Разберись,<br>как <em>думают потоки</em></h1><p>Последовательный курс по многопоточности в Java: от первого потока и race condition до Java Memory Model, конкурентных коллекций и виртуальных потоков.</p><div class="hero-actions"><a class="primary" href="${base}chapters/${chapters[0].slug}/">Начать обучение <span>→</span></a><a href="#route">Посмотреть программу</a></div><div class="code-art" aria-hidden="true"><span>Thread.ofVirtual()</span><strong>.start(task);</strong><i></i><i></i><i></i></div></section><section class="stats"><div><b>${chapterCount}</b><span>связанная глава</span></div><div><b>≈ 3 ч</b><span>на весь маршрут</span></div><div><b>Java</b><span>примеры из практики</span></div></section><section id="route" class="route"><div class="section-title"><span>01 — ${chapterCount}</span><div><h2>Учебный маршрут</h2><p>Иди по порядку: каждая следующая тема опирается на предыдущую.</p></div></div><div class="route-list">${cards}</div></section>`;
